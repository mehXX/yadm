import { KarabinerRules, KeyCode, LayerModifier, Manipulator, To } from "./types";
...
export function createHyperSubLayer(
  ...
  return [
    ...
    {
      description: `Toggle ${layer_modifier} sublayer ${sublayer_key}`,
      type: "basic",
      from: {
...
export function createKeySubLayers(
  layer_modifier: LayerModifier,
  subLayers: { [key_code in KeyCode]?: HyperKeySublayer | LayerCommand; }
): KarabinerRules[] {
  ...
  return Object.entries(subLayers).map(([key, value]) =>
    "to" in value
      ? {
        description: `${layer_modifier} key + ${key}`,
        manipulators: [
          {
            ...value,
            type: "basic" as const,
      ...
      : {
        description: `${layer_modifier} key sublayer "${key}"`,
...
export function GenerateMapFromTo(key_code_from: string, key_code_to: string): Manipulator {
  return {
    type: "basic",
    from: {
      key_code: key_code_from as KeyCode,
    },
    to: [
      {
        key_code: key_code_to as KeyCode,
      },
    ],
  };
}
